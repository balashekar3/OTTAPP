...
func generateMovies(_ count: Int) -> [Movie] {
    ...
    for _ in 0..<count {
        ...
        let defEpiInfo = CurrentEpisodeInfo(episodeName: epiname, decription: epidesc, season: 1, episode: 1)
...
func generateEpisodes(_ count: Int, forSeason season: Int) -> [Episods] {
    var allEpisodes: [Episods] = []
    ...
    for num in 0..<count {
        ...
        let episode = Episods(id: id, name: name, season: season, episodeNumber: num + 1, thumbnailImageURLString: thumbnail, discription: desc, length: randLength, videoURL: exampleVideoURL)
