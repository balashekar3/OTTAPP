...
struct SearchView: View {
    ...
    var body: some View {
        ...
        return ZStack{
            ...
            VStack{
                SearchBar(text: searchTextBinding, isLoading: $vm.isLoadingSearch).padding()
                ScrollView{
                    ...
                    if vm.viewState == .empty{
                        ...
                        VStack{
                            HStack{
                                Text("Movies & TV").bold().font(.title3).padding(.leading,12)
                                Spacer()
                            }
                            SearchResultGridView(movies: vm.searchResults,
                                                 movieDetailsToShow: $movieDetailToShow
                            )
                        }
                    }
                }
...
struct PopularList:View{
    ...
    var body: some View {
        VStack {
            HStack {
                Text("Popular Searches").bold().font(.title3).padding(.leading,20)
                Spacer()
            }
            LazyVStack{
